    ...
    wsl::shared::SocketChannel& Channel, wsl::shared::Transaction& Transaction, const WSLC_TTY_RELAY& Message, const gsl::span<gsl::byte>&)
{
    ...
    while (true)
    {
        ...
        {
            auto [ttyMessage, _] = TerminalControlChannel.ReceiveMessageOrClosed<WSLC_TERMINAL_CHANGED>();
    ...
    wsl::shared::SocketChannel& Channel, wsl::shared::Transaction& Transaction, const WSLC_FORK& Message, const gsl::span<gsl::byte>& Buffer)
{
    ...
    Response.Header.MessageSize = sizeof(Response);
    Response.Header.MessageType = WSLC_FORK_RESULT::Type;
...

    {
        ...
        else if (Message.ForkType == WSLC_FORK::Process)
        {
            Response.Pid = UtilCreateChildProcess("CreateChildProcess", std::move(childLogic));
        }
        else if (Message.ForkType == WSLC_FORK::Pty)
        {
    ...
    wsl::shared::SocketChannel& Channel, wsl::shared::Transaction& Transaction, const TMessage& Message, const gsl::span<gsl::byte>& Buffer)
{
    ...
    try
    {
        ...
        if constexpr (std::is_same_v<TMessage, WSLC_MOUNT>)
        {
            if (WI_IsFlagSet(Message.Flags, WSLC_MOUNT::KernelModules))
            {
...
int WSLCEntryPoint(int Argc, char* Argv[])
{
